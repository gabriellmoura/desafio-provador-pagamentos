...
public class CenarioRecebiveisComAdiantamento {
	...
	void teste(List<DadosTesteTransacao> infos, List<String> adiantamentos,
			List<DadosEsperadosRetorno> retornosEsperados) throws Exception {
		ArrayList<String> paramsTransacoes = new ArrayList<>();
		for (DadosTesteTransacao info : infos) {
			String param = String.format("%s,%s,%s,%s,%s,%s,%s", info.valorTransacao, info.formaPagamento, "764387534",
					"Nome do cartao", LocalDate.now().plusYears(2).format(DateTimeFormatter.ofPattern("dd/MM/yyyy")),
					"457", info.idTransacao);
		...
		for (int i = 0; i < retornos.size(); i++) {
			...
			String valorASerRecebido = new BigDecimal(retornos.get(i)[2]).setScale(0, RoundingMode.HALF_EVEN)
					.toString();
	...
	static Stream<Arguments> geradorDeTransacoesComAdiantamento() {
		...
		DateTimeFormatter padraoFormatacao = DateTimeFormatter.ofPattern("dd/MM/yyyy");

		return Stream.of(Arguments.of(List.of(new DadosTesteTransacao("100", "CREDITO", 1)), List.of("1,0.01"),
				List.of(new DadosEsperadosRetorno("pago", "100", "86", hoje.plusDays(30).format(padraoFormatacao))

				)),
				Arguments.of(
						List.of(new DadosTesteTransacao("100", "CREDITO", 1),
								new DadosTesteTransacao("200", "CREDITO", 2)),
						List.of("1,0.10", "2,0.10"),
						List.of(new DadosEsperadosRetorno("pago", "100", "86", hoje.format(padraoFormatacao)),
								new DadosEsperadosRetorno("pago", "200", "171", hoje.format(padraoFormatacao))
